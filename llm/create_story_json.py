...
[작업 규칙]
1.  **장면 나누기:** 이야기의 전통적인 구조(기승전결)를 참고하여 4~5개의 핵심 장면(Moment)으로 나누고, 각 장면에 고유한 영어 ID(예: MOMENT_START)를 붙여주세요.

2.  **분기 생성:** 플레이어의 선택이 의미 있도록, 원작에 없더라도 선택의 결과로 이어질 '새로운 장면'이나 '짧은 엔딩'(좋은/나쁜/재미있는 엔딩 등)을 1~2개 이상 창의적으로 만들어내야 합니다. 단, 모든 새로운 분기는 원작의 핵심 교훈을 강화하거나, 등장인물의 성격을 더 깊이 탐구하는 방향으로 만들어져야 합니다.

3.  **장면 묘사 원칙 (클리프행어):** 선택지가 있는 장면(엔딩이 아닌 장면)의 'description'은, 반드시 플레이어가 선택을 내리기 직전의 긴장감 넘치는 상황까지만 묘사해야 합니다. 선택의 결과를 미리 암시하거나 결론을 내리면 절대 안 됩니다.
    *   (예시): "주인공은 동굴 깊은 곳에서 거대한 무언가가 천천히 눈을 뜨는 것을 보았다." 처럼, "그래서 어떻게 됐을까?" 하고 궁금해하는 순간에 묘사를 멈춰야 합니다.

4.  **논리적 일관성 검증 (인과관계):** 선택지는 '원인(Cause)', 이어지는 장면의 내용은 '결과(Effect)'입니다. 이 둘은 반드시 명확하고 설득력 있는 인과관계로 이어져야 합니다. '친구를 구하러 간다'는 선택지가 '혼자 보물을 발견하는' 장면으로 이어지는 것처럼, 논리적으로 말이 안 되는 연결은 절대 만들면 안 됩니다.

5.  **완벽한 기술적 연결:** 각 'choices' 배열 안의 모든 선택지는, 반드시 'next_moment_id' 키를 통해 이 JSON 파일 내에 실제로 '정의된' 다른 장면 ID로 연결되어야 합니다. 이것은 매우 중요한 기술적 규칙입니다.

6.  **엔딩 처리:** 이야기의 끝을 맺는 장면(엔딩)에는 'choices' 키 자체를 포함하지 마세요. 엔딩의 'description'은 최종적인 결과와 이야기가 주는 교훈을 요약해야 합니다.

7.  **JSON 형식 준수:** 최종 결과는 반드시 아래 [출력 JSON 형식]과 똑같은 구조의 JSON 데이터로만 출력해야 합니다. 설명이나 다른 말을 절대 덧붙이지 마세요.
