...
def get_scene_templates(request):
    """
    [수정] 모든 씬 데이터는 LLM/WebSocket이 처리하므로, 템플릿이 없음을 알립니다.
    """
    # 템플릿 기반 씬 데이터를 사용하지 않는다는 메시지를 반환합니다.
    return JsonResponse({"scenes": [], "message": "All scene data is dynamically generated by LLM/WebSocket. No static templates found."}, status=200)
